        ...
        <div className="flex flex-col gap-4 md:gap-5 relative">
            <div className="flex justify-between w-full relative">
                ...
                {purchasedSkills.includes('skill-scan') && (
                    <div ref={scanRefillRef} className="relative flex flex-col items-center">
                        <button 
                            ...
                            className={`flex flex-col items-center gap-1 md:gap-1.5 transition relative ${getBaseButtonStyle(!scanCooldown && !isScanning)}`}
                        ...
                        >
                        <div className={`p-3 md:p-4 rounded-full shadow-sm transition-all duration-300 relative flex items-center justify-center ${getBaseContainerStyle(!scanCooldown && !isScanning)}`}>
                            <Icons.Scan className={`w-5 h-5 md:w-6 md:h-6 scale-[1.16] ${!scanCooldown && !isScanning ? 'opacity-100' : 'opacity-30 grayscale'}`} />
                            {scanUses > 0 ? (
                        ...
                        <span className="text-sm md:text-base font-medium">Scan</span>
                    </button>
                    <AnimatePresence>
                        {showScanRefill && scanUses === 0 && (
                            <div className="absolute top-[calc(100%+8px)] left-0 z-[160]">
                                <motion.div
                                    ...
                                    className="relative w-[172px] md:w-[210px] rounded-2xl border border-stone-200 dark:border-stone-700 bg-white dark:bg-stone-800 px-3 md:px-4 py-2.5 md:py-3.5 text-center shadow-lg"
                                    role="dialog"
                                    aria-label="Buy an extra Scan"
                                >
                                    <div className="text-[12px] md:text-sm font-semibold text-t-primary leading-snug">
                                        Use 1 Scan for{' '}
                                    ...
                                    {currentPoints < scanRefillCost && (
                                        <div className="mt-1 text-[10px] md:text-xs font-semibold text-red-500">
                                            Not enough diamonds
                                        </div>
                                    )}
                                    <div className="mt-2 flex items-center gap-1.5">
                                        <button
                                            ...
                                            className="flex-1 rounded-full bg-stone-100 dark:bg-stone-700 px-2 py-1.5 md:py-2 text-[11px] md:text-[13px] font-bold text-t-secondary active:scale-95 transition"
                                        >
                                            Cancel
                                        </button>
                                        <button
                ...
                >
                    ...
                    <span className="text-sm md:text-base font-medium">Undo</span>
                </button>
                ...
                >
                    ...
                    <div className={`p-3 md:p-4 rounded-full shadow-sm transition-colors ${
                        ...
                        <Icons.Pencil className={`w-5 h-5 md:w-6 md:h-6 scale-125 ${isPencilMode ? 'text-blue-600 dark:text-blue-400' : 'text-stone-700 dark:text-stone-300'}`} />
                    </div>
                    <span className="text-sm md:text-base font-medium">Pencil</span>
                </button>
                ...
                >
                    <div className={`p-3 md:p-4 rounded-full shadow-sm transition-colors ${
                        ...
                        <Icons.Erase className={`w-5 h-5 md:w-6 md:h-6 scale-125 ${
                            isEraseMode
